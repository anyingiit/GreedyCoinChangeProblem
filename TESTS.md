...
- Cases / 用例：
    - `expectSuccessWithBreakdown`: Verifies a feasible stock returns the expected greedy breakdown and exact sum to
      target (186). / 验证可行库存能得到预期的贪心分解，且总额等于目标 186。
...
- Run statistical validation (example 10k trials): `./build/GreedyCoinChangeValidation 10000 8 20000`. / 运行统计验证（示例
  1 万次）：`./build/GreedyCoinChangeValidation 10000 8 20000`。
